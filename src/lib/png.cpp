  ...
  type_(PNG_COLOR_TYPE_NONE)
{
  ...
  case PNG_COLOR_TYPE_PALETTE:
    png_set_palette_to_rgb(png.hdl());
    break;

  case PNG_COLOR_TYPE_GRAY:
  case PNG_COLOR_TYPE_GRAY_ALPHA:
    png_set_gray_to_rgb(png.hdl());
    break;
  }
...
int Png::type() const { return type_; }

// サイズ
int Png::width() const { return width_; }
int Png::height() const { return height_; }

// ピクセルデータ
const u_char* Png::image() const { return &image_[0]; }


// png読み込み時のコールバック
void Png::readFunc(png_struct* hdl, png_bytep buf, png_size_t size) {
